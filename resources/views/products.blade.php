...
@section('title', 'Nuestros Productos - Amplia Gama Textil | Importadora Camila')
...
<section class="hero-subpage d-flex align-items-center justify-content-center text-white" style="background-image: url('https://images.unsplash.com/photo-1512436991641-6745cdb1723f?auto=format&fit=crop&w=1600&q=80');" data-aos="fade-in">
    <div class="container text-center py-5">
        ...
        <div class="d-flex gap-3 justify-content-center flex-wrap" data-aos="fade-up" data-aos-delay="300">
            <a href="{{ $catalogAnchor }}" class="btn btn-outline-light btn-lg">Explorar catálogo</a>
            <a href="{{ route('client.login') }}" class="btn btn-accent btn-lg">Portal de reservas</a>
        </div>
    </div>
</section>
...
<section class="py-5 {{ $loop->even ? 'bg-white' : 'bg-light-gray' }}" id="{{ \Illuminate\Support\Str::slug($category) }}">
    <div class="container">
        <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-lg-between mb-4" data-aos="fade-up">
            <div>
                <h2 class="display-6 fw-bold text-primary-dark mb-2">{{ $category }}</h2>
                <p class="text-secondary-dark mb-0">{{ $items->first()->collection ?? 'Colección destacada Camila Textil' }}</p>
            </div>
            <a href="{{ route('client.login') }}" class="btn btn-outline-accent mt-3 mt-lg-0">Reservar desde el portal</a>
        </div>
        <div class="row g-4">
            @foreach($items as $product)
                <div class="col-md-6 col-xl-4" data-aos="zoom-in" data-aos-delay="{{ $loop->index * 100 }}">
                    <div class="card h-100 shadow-sm border-0 product-card">
                        @php($highlight = $product->colors->first())
                        <div class="ratio ratio-4x3">
                            <img src="{{ optional($highlight)->image_url }}" alt="Vista de {{ $product->name }}" class="w-100 h-100 object-fit-cover">
                        </div>
                        <div class="card-body d-flex flex-column">
                            <span class="badge bg-accent text-primary-dark mb-2">{{ $product->fabric_type }}</span>
                            <h3 class="h5 fw-bold text-primary-dark">{{ $product->name }}</h3>
                            <p class="small text-secondary-dark flex-grow-1">{{ \Illuminate\Support\Str::limit($product->description, 160) }}</p>
                            <ul class="list-unstyled small text-secondary-dark mb-3">
                                ...
                                @if($product->lead_time_days)
                                    <li><i class="fas fa-clock text-accent me-2"></i>Reposición en {{ $product->lead_time_days }} días</li>
                                @endif
                            </ul>
                            <div class="d-flex flex-wrap gap-2 mb-3">
                                @foreach($product->colors as $color)
                                    <button type="button" class="btn btn-outline-primary btn-sm d-flex align-items-center gap-2"
                                        ...
                                        data-stock="{{ $color->available_meters ? $color->available_meters.' m disponibles' : $color->available_rolls.' rollos disponibles' }}"
                                        data-image="{{ $color->image_url }}">
                                        <span class="d-inline-block rounded-circle border" style="width:14px;height:14px;background-color: {{ $color->hex_code ?? '#ffffff' }}"></span>
                                        <span>{{ $color->name }}</span>
                                    </button>
                                @endforeach
                            </div>
                            <a href="{{ route('client.login') }}" class="btn btn-accent btn-sm mt-auto">Reservar este tejido</a>
                        </div>
                    </div>
                </div>
            @endforeach
                ...
                <div class="modal-body">
                    <div class="row g-4 align-items-center">
                        ...
                        <div class="col-lg-6">
                            ...
                            <a href="{{ route('client.login') }}" class="btn btn-accent">Reservar este color</a>
                        </div>
                    </div>
                </div>
